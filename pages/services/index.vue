  ...
  <div class="flex flex-wrap justify-center">
    <template v-for="(service, key) in services">
      <NuxtLink
        ...
        aria-haspopup="true"
        aria-expanded="true"
        class="p-2 text-gray-700 transition-colors rounded-md hover:text-blue-500 w-1/3 h-48"
      >
        <div class="flex flex-col h-full bg-gray-50 hover:bg-gray-100 justify-around items-center p-4 shadow text-center w-full">

          <svg class="fill-current h-16 w-16">
            <use :xlink:href="service.icon" />
          </svg>
          <span class="text-sm">{{ service.title }}
            <span v-if="service.price" class="">по цене {{ service.price }}</span>
          </span>
        </div>
      </NuxtLink>
    </template>
    <svg class="hidden">
      ...
      <symbol viewBox="0 0 24 24" id="file-image">
      ...
      <symbol viewBox="0 0 24 24" id="file-document-edit-outline">
      ...
      <symbol viewBox="0 0 24 24" id="card-account-details">
      ...
      <symbol viewBox="0 0 24 24" id="file-image-marker">
